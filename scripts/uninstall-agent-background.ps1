...
$fileErrors = @(Invoke-SswBestEffortPlan -Plan @(
    ...
    [pscustomobject]@{ Name = 'remove-data'; Action = {
        if ($RemoveData -and (Test-Path -LiteralPath $data)) {
            ...
            Write-Warning 'Agent identity and local background configuration were removed and cannot be recovered.' -WarningAction Continue
        }
    } }
))
...
if (-not $RemoveData) {
    Write-Host "HTTPS 신원, 설정과 legacy 보존 자료는 다음 설치에 재사용할 수 있도록 보존했습니다: $data"
}
