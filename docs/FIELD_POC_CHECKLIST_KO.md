...
- [ ] Agent와 Viewer 파일명이 같은 `0.11.2-poc` 버전을 표시함
...
- [ ] 복구 성공 뒤 운영자가 `설치/업데이트`를 한 번 눌러 내부 사전 점검부터 새 작업을 별도로
      실행함
