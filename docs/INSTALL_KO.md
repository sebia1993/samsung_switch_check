...
4. 별도 주소나 CIDR 입력 없이 `설치/업데이트`를 한 번 누릅니다. Setup이 하나의 트랜잭션
   안에서 필요한 검사를 수행한 뒤 설치 또는 업데이트를 계속합니다.
...
`설치/업데이트`를 누르면 Setup이 별도 사전 점검을 중복 실행하지 않고 설정 변경 전에 다음
단계를 같은 설치 작업 안에서 확인하며, 통과하거나 계속 가능한 경고인 경우에만 설치를
진행합니다.
...
확인된 경우에만 파일 작업으로 진행합니다.
신규 설치 복구에서 서비스를 삭제할 때도 Windows에서 실제 삭제가 확인된 뒤 완료 처리합니다.
...
1. Agent Setup에서 `설치/업데이트`를 한 번 눌러 같은 작업의 내부 검사와 설치를 완료합니다.
...
- 복구 성공은 설치 성공이 아닙니다. 설치 버튼이 다시 활성화되면 운영자가
  `설치/업데이트`를 한 번 눌러 같은 설치 작업의 내부 검사부터 새 작업을 시작합니다.
...
1. Agent PC에서 Setup의 `설치/업데이트`를 한 번 눌러 같은 작업의 내부 검사와 설치를 완료합니다.
