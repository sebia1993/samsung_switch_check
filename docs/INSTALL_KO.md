...
공식 GitHub `v0.11.0-poc` Release의 Assets에서 다음 두 파일만 받습니다.
...
`0.11.0-poc`는 코드 서명되지 않은 시험판입니다. SmartScreen, EDR, AppLocker 또는 WDAC가
...
`0.11.0-poc` Agent ZIP의 Setup에서 별도의 `이전 상태 복구`를 사용하십시오. 복구 성공 뒤에는
...
- `0.11.0-poc` Agent는 서비스 시작마다 새 RSA 인증서를 생성합니다. Windows Schannel용 임시
...
확인한 뒤 `0.11.0-poc` 패키지의 `설치/업데이트`를 한 번만 다시 실행하십시오. Setup이 내부
