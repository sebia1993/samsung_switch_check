...
```powershell
.\scripts\build-release.ps1 -Version 0.9.20-poc
```
...
```powershell
.\scripts\build-release.ps1 -Version 0.9.20-poc -AllowDirty
```
...
```powershell
git tag -a v0.9.20-poc -m "Samsung Switch Watch v0.9.20-poc"
git push origin v0.9.20-poc
```
