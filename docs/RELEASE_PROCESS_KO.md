...
예를 들어 `0.5.1-poc`를 게시할 때는 검증된 커밋에 annotated tag를 만들고 한 번만 push합니다.

```powershell
git tag -a v0.5.1-poc -m "Samsung Switch Watch 0.5.1 POC"
git push origin v0.5.1-poc
```
...
gh attestation verify .\SamsungSwitchWatch-Agent-0.5.1-poc-win-x64.zip `
  ...
  --source-ref refs/tags/v0.5.1-poc `
  --deny-self-hosted-runners
gh release verify v0.5.1-poc --repo sebia1993/samsung_switch_check
gh release verify-asset v0.5.1-poc `
