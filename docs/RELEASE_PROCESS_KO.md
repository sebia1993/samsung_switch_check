...
```powershell
.\scripts\build-release.ps1 -Version 0.9.19-poc
```
...
```powershell
.\scripts\build-release.ps1 -Version 0.9.19-poc -AllowDirty
```
...
```powershell
git tag -a v0.9.19-poc -m "Samsung Switch Watch v0.9.19-poc"
git push origin v0.9.19-poc
```
