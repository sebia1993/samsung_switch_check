...
## v0.7 Agent API의 명시적 제한

`v0.7.0-poc`의 Agent–Viewer 연결은 일반 `HTTP`이며 TLS 암호화와 사용자 인증이 없습니다.
인증서 SHA-256 pin, 페어링 코드, Bearer 토큰 기능은 제거되었습니다. 따라서 Agent API에
도달할 수 있는 호스트는 상태 조회뿐 아니라 수동 점검 요청, 이벤트 확인, 그리고 설치 시 켠
읽기 전용 장비 명령 API도 사용할 수 있습니다. 장비 명령 결과도 HTTP 구간에서 암호화되지
않으므로 고정 Viewer IPv4 방화벽과 격리 관리망이 반드시 필요합니다.
...
- 옵트인 장비 명령과 Viewer 메모리에 일시적으로 표시된 결과
...
- 주기 수집과 기존 수동 점검은 모델 프로파일에 등록된 `show` 명령 ID만 실행합니다.
- Viewer 장비 명령은 `Agent:EnableReadOnlyQueries=false`가 기본값이며 설치 시
  `-EnableReadOnlyQueries`를 명시한 경우에만 활성화합니다.
- 활성화해도 Agent가 128자 이하 한 줄 `show` 명령, 승인된 첫 토큰, 금지 키워드와 구분자를
  자격 증명 조회나 TCP 연결 전에 다시 검사합니다.
- `running-config`, `startup-config`, 계정·AAA·RADIUS·TACACS·SNMP community·비밀번호·secret·
  key·crypto 관련 조회와 제어문자, 줄바꿈, 세미콜론·파이프·앰퍼샌드·백틱·달러·꺾쇠
  구분자는 항상 차단합니다.
- 같은 장비의 주기 수집과 공통 잠금을 사용하고 5초 안에 잠금을 얻지 못하면 거부합니다.
  Viewer IPv4별 분당 최대 12회, 전체 요청 최대 60초이며 대기열은 만들지 않습니다.
- 관리자용 Agent 서비스는 `LocalService`와 서비스 SID 전용 ACL로 실행합니다.

## 현재 사용자 숨김 모드의 제한

`install-agent-background.ps1`은 관리자 권한을 받을 수 없는 PC를 위한 대체 실행 방식입니다.
Agent를 `%LOCALAPPDATA%`에 설치하고 현재 사용자 `InteractiveToken`·`LeastPrivilege` 예약 작업으로
창 없이 실행합니다. Windows 서비스 방식과 동시에 사용할 수 없습니다.

- `LocalService` 계정 격리와 서비스 SID 전용 ACL을 사용하지 않습니다.
- 같은 Windows 계정을 사용하는 사람은 프로세스를 종료하거나 숨김 예약 작업을 끌 수 있습니다.
- RDP 연결 해제 중에는 계속 실행되지만 로그오프 중에는 실행되지 않습니다.
- 설치기는 Windows 방화벽을 생성·수정·삭제하지 않습니다.
- 기존 관리자 승인 방화벽에서 정확한 Viewer 고정 IPv4만 허용된 환경에 한해 사용합니다.

따라서 이 모드는 화면 오조작을 줄이는 가용성 기능이며 권한이 있는 같은 계정 사용자에 대한
보안 경계가 아닙니다. 관리자 승인을 받을 수 있으면 Windows 서비스 방식을 우선합니다.

## 저장 보호

스위치 자격 증명과 Telnet 원문은 Windows DPAPI `LocalMachine` scope로 보호합니다. 서비스
방식의 데이터 디렉터리 ACL은 서비스 SID와 관리자로 제한합니다. 현재 사용자 숨김 방식은
사용자 프로필의 `%LOCALAPPDATA%\SamsungSwitchWatch\AgentData`에 저장하며 같은 Windows 계정의
권한을 따릅니다. DB만 다른 PC로 복사해서는 복호화할 수 없습니다.

주기 수집 원문은 Agent 전용이며 API와 Viewer로 반환하지 않습니다. 기본 보존은 7일·500MB입니다.
옵트인 장비 명령은 정규화된 결과만 UTF-8 65,536바이트까지 요청한 Viewer에 반환합니다. 결과는
Agent snapshot/event/raw DB와 Viewer 설정·명령 이력에 저장하지 않고 CSV/JSON 내보내기에도
포함하지 않습니다. 감사에는 장비 ID, Viewer IP, 명령 SHA-256, 소요 시간, 결과 코드와 출력
크기만 저장하며 명령 문자열과 결과 본문은 저장하지 않습니다. Viewer export는 실제 device ID
대신 익명 별칭을 사용하며 IP, 사용자명과 원문을 제외합니다.
...
아래 항목은 관리자용 Windows 서비스 설치에 적용됩니다. 현재 사용자 숨김 설치는 방화벽을
변경하지 않으며, 이미 승인된 동일 수준의 고정 IPv4 정책이 있어야 합니다.
...
`STORAGE_WRITE_FAILED`, `QUERY_DISABLED`, `QUERY_COMMAND_BLOCKED`, `DEVICE_BUSY`,
`QUERY_RATE_LIMITED`, `QUERY_TIMEOUT`.
