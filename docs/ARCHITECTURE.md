...
4. `허용할 Viewer PC 고정 IPv4 · Agent PC 주소 아님`에 원격 Viewer PC의 주소 한 개를
   입력합니다. 같은 PC에서 시험할 때만 `같은 PC 시험용 주소`로 Agent PC의 실제 RFC1918
   사설 IPv4를 선택합니다.
...
- 서비스·파일·로컬 HTTPS/API/버전 확인 실패 시 프로그램, 서비스와 방화벽 상태 rollback 시도
- 방화벽·GPO·규칙 적용/재조회 실패만 발생하면 방화벽 변경분 복원을 시도하고 Agent 설치는 유지

새 서비스 파일을 활성화한 뒤에는 자동 복구 정책을 일시적으로 비활성화하고, SCM readback에서
재시작 작업이 0개인지 확인한 뒤 서비스를 시작합니다. readiness 검사는 매 시도마다 SCM에서
현재 서비스 PID를 다시 읽어 그 프로세스가
TCP/18443을 소유하는지 확인합니다. 시작 중 PID가 바뀌어도 현재 프로세스의 HTTPS 응답이
제품·API·프로토콜 계약과 일치하면 성공으로 인정합니다. 그 뒤에만 5초·15초·60초 재시작
정책을 적용하고 설치를 commit합니다. 따라서 예약된 서비스 재시작이 readiness 또는 rollback과
겹치지 않습니다.
...
오류에 포함하지 않습니다. 이 경우 방화벽 변경분은 기존 snapshot으로 복구를 시도하고 결과를
경고에 남기되 Agent 서비스와 프로그램은 유지합니다. 로컬 HTTPS/API/버전 준비 상태가 성공해야만 이 경고 완료 상태를
사용할 수 있습니다. 로컬 준비 상태 실패는 계속 `SETUP_HEALTH_FAILED`와 전체 rollback입니다.
...
`같은 PC 시험용 주소`는 현재 PC에서 활성 상태인 loopback·tunnel 이외 RFC1918 IPv4를 찾습니다.
...
확인할 수 없으면 `FIREWALL_REMOTE_ACCESS_UNCONFIRMED`로 설치를 완료합니다. Agent는 입력한
Viewer IPv4를 계속 재검증하며 Setup은 `Any`, `LocalSubnet` 또는 넓은 대역 규칙을 만들지
않습니다. 원격 도달 가능성은 Viewer 연결 진단으로 최종 확인합니다.
...
검사·설치·복구 작업이 성공 또는 실패로 끝나면 `익명 진단 저장`을 눌러
...
3. Agent PC의 IPv4를 입력하고 연결을 확인합니다. 같은 PC 사전 검증은 운영자가
   `Agent와 Viewer가 같은 PC일 때 테스트`를 눌렀을 때만 실행합니다.
...
연결과 같은 PC 시험 여부, 주소·DNS·TCP·HTTPS·Identity 단계별 상태와 제한된 소요 시간,
...
Viewer가 Agent 연결 설정을 바꿀 때는 새 Agent의 준비 상태와 identity를 먼저 확인하고 설정을
