...
삼성 스위치 여러 대 ── Telnet/23 ──> SamsungSwitchWatch.Agent
                                          │ HTTP/18443 + SignalR
                                          │ (암호화·인증 없음, 고정 IP 방화벽 제한)
                                          ▼
                                  SamsungSwitchWatch.Viewer
```

현재 버전은 `v0.7.0-poc`입니다. 실제 펌웨어 3종을 회사망에서 검증하기 전까지는
...
- 기본 비활성화된 Viewer 장비 명령: Agent가 허용한 한 줄 `show` 조회만 실행하고 결과는 Viewer 메모리에만 유지
- `show port status`, `show syslog tail num 100` 우선 실행과 현장 `show sylog tail num 100` 표기 및 모델별 자동 대체 명령
...
- Agent/API/실시간/DB/수집기 상태를 구분한 Figma 기반 대시보드
...
- DPAPI 스위치 자격 증명·Telnet 원문 보호
- Agent 주소와 포트만 입력하는 간단한 Viewer 연결 화면
- 1~32개 고정 Viewer IPv4만 허용하는 Windows 방화벽과 원자적 허용 목록 갱신
- 관리자 권한이 없을 때 현재 사용자 로그온 동안 창 없이 실행되는 예약 작업 설치 모드
...
Agent PC에서 관리자 승인을 받을 수 없으면 보이는 Agent를 한 번 종료한 뒤 일반 PowerShell에서
다음 명령으로 현재 사용자용 숨김 예약 작업을 설치할 수 있습니다.
...
이 모드는 RDP 연결 해제 중에는 계속 실행되지만 Windows 로그오프 중에는 중지되며, 방화벽을
변경하지 않습니다. 상세 조건과 제거 방법은 [설치 안내](docs/INSTALL_KO.md)를 확인하십시오.

## 문서

- [설치·복구·Viewer 연결](docs/INSTALL_KO.md)
...
- [0.7.0-poc 릴리스 노트](docs/RELEASE_NOTES_0.7.0_POC_KO.md)
- [Figma handoff](docs/figma/README.md)

## 보안 경계

Telnet과 Agent–Viewer HTTP는 모두 암호화되지 않습니다. Agent API에는 사용자 인증도 없습니다.
두 경로를 격리된 사내 관리망에만 두고, 설치 시 등록한 고정 Viewer IPv4 방화벽 범위를 유일한
접근 통제로 유지하십시오. 비밀번호, 실제 IP, 호스트명, MAC, 시리얼과 회사 로그를 저장소·이슈·
외부 진단 자료에 넣지 마십시오.

Viewer 장비 명령은 설치 시 명시적으로 켠 경우에만 사용할 수 있습니다. Agent는 한 줄 `show`
허용 목록을 다시 검증하고 위험 키워드와 구분자를 차단합니다. 정규화된 결과는 최대 64KiB만
요청한 Viewer 메모리에 반환되며 DB 저장과 내보내기를 지원하지 않습니다. 실제 스위치 설정
변경·재부팅·로그 삭제 기능은 포함하지 않습니다.
