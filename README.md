...
현재 버전은 `v0.8.0-poc`입니다. 실제 세 모델의 펌웨어에서 검증하기 전까지는
...
Viewer ZIP은 운영자 PC에서 다음과 같이 설치합니다.

```powershell
.\install-viewer.ps1 -SourceDirectory . -StartWithWindows
```

기존 Viewer를 업데이트할 때 `-StartWithWindows`를 생략하면 현재 자동 시작 상태를
보존합니다. 자동 시작을 끌 때만 `-DisableStartWithWindows`를 지정합니다.
...
- [0.8.0-poc 릴리스 노트](docs/RELEASE_NOTES_0.8.0_POC_KO.md)
